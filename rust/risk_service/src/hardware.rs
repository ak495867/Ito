use crate::generated::{
    FRAME_BYTES as GENERATED_FRAME_BYTES, OFFSET_CONTROL, OFFSET_HEALTH, OFFSET_LIMITS_VERSION,
    OFFSET_MAX_NET_POSITION, OFFSET_MAX_NOTIONAL_TICKS, OFFSET_MAX_QUANTITY, OFFSET_NET_POSITION,
    OFFSET_PRICE_TICKS, OFFSET_QUANTITY,
};
...
pub fn encode_frame(frame: FpgaRiskFrame) -> [u8; FRAME_BYTES] {
    ...
    put_u64(
        &mut bytes,
        OFFSET_MAX_NOTIONAL_TICKS,
        frame.max_notional_ticks,
    );
...
mod tests {
    ...
    fn frame_uses_generated_register_offsets() {
        ...
        assert_eq!(
            u64::from_le_bytes(
                bytes[OFFSET_PRICE_TICKS..OFFSET_PRICE_TICKS + 8]
                    .try_into()
                    .unwrap()
            ),
            100
        ...
        assert_eq!(
            u64::from_le_bytes(
                bytes[OFFSET_QUANTITY..OFFSET_QUANTITY + 8]
                    .try_into()
                    .unwrap()
            ),
            10
        ...
        assert_eq!(
            u64::from_le_bytes(
                bytes[OFFSET_MAX_QUANTITY..OFFSET_MAX_QUANTITY + 8]
                    .try_into()
                    .unwrap()
            ),
            100
        ...
        assert_eq!(
            u64::from_le_bytes(
                bytes[OFFSET_MAX_NOTIONAL_TICKS..OFFSET_MAX_NOTIONAL_TICKS + 8]
                    .try_into()
                    .unwrap()
            ),
            10_000
        ...
        assert_eq!(
            i64::from_le_bytes(
                bytes[OFFSET_NET_POSITION..OFFSET_NET_POSITION + 8]
                    .try_into()
                    .unwrap()
            ),
            0
        ...
        assert_eq!(
            u64::from_le_bytes(
                bytes[OFFSET_MAX_NET_POSITION..OFFSET_MAX_NET_POSITION + 8]
                    .try_into()
                    .unwrap()
            ),
            1_000
        ...
        assert_eq!(
            u64::from_le_bytes(
                bytes[OFFSET_LIMITS_VERSION..OFFSET_LIMITS_VERSION + 8]
                    .try_into()
                    .unwrap()
            ),
            1
