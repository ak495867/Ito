use ito_risk_service::{
    evaluate,
    hardware::{self, HardwarePipeline},
    RiskRequest,
};
use std::env;
...
fn respond(request: &str) -> String {
    match serde_json::from_str::<RiskRequest>(request) {
        Ok(value) => serde_json::to_string(&evaluate(value))
...
fn main() -> io::Result<()> {
    if env::args().any(|argument| argument == "--emit-frame") {
        ...
        let request = serde_json::from_str::<RiskRequest>(input.trim())
            .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "request_invalid"))?;
        let pipeline = HardwarePipeline::new();
        let (frame, response) = pipeline.prepare(request);
        println!(
            "{}",
            serde_json::json!({"frame_hex": hardware::to_hex(&frame), "response": response})
        );
        return Ok(());
    }
