...
class ConnectivityTests(unittest.TestCase):
    ...
    def test_reconciliation_blocks_mismatch(self):
        local = [{"client_order_id": 1, "executed_quantity": 10, "average_price_ticks": 100}]
        venue = [{"client_order_id": 1, "executed_quantity": 5, "average_price_ticks": 100}]
    ...
    def test_route_recommendation_filters_unready_venues(self):
        routes = [{"venue_id": 5, "rank": 1, "fee_bps": 2, "enabled": True, "live_enabled": False, "max_order_quantity": 100}, {"venue_id": 6, "rank": 2, "fee_bps": 3, "enabled": True, "live_enabled": False, "max_order_quantity": 100}]
        sessions = [{"venue_id": 5, "status": "degraded"}, {"venue_id": 6, "status": "ready"}]
    ...
    def test_venue_status_requires_all_sessions_ready(self):
        self.assertTrue(aggregate([{"venue_id": 5, "status": "ready"}, {"venue_id": 6, "status": "ready"}])["execution_enabled"])
        self.assertFalse(aggregate([{"venue_id": 5, "status": "ready"}, {"venue_id": 6, "status": "uncertain"}])["execution_enabled"])
