...
class Portfolio:
    def __init__(self, limits: PortfolioLimits) -> None:
        if min(limits.max_net_position, limits.max_gross_position, limits.max_gross_notional_ticks, limits.max_concentration_ticks) <= 0 or limits.max_loss_ticks < 0:
    ...
    def apply_fill(self, instrument_id: int, side: int, quantity: int, price_ticks: int) -> Position:
        if side not in (-1, 1) or quantity <= 0 or price_ticks <= 0:
            raise PortfolioError("fill_invalid")
        position = self._position(instrument_id)
        if side < 0 and position.quantity >= 0 and quantity > position.quantity and not self.limits.allow_short:
        ...
        if old_quantity == 0 or (old_quantity > 0 and signed_quantity > 0) or (old_quantity < 0 and signed_quantity < 0):
            ...
            position.average_price_ticks = (abs(old_quantity) * old_average + quantity * price_ticks) // total_quantity
            position.quantity = old_quantity + signed_quantity
        else:
            closing_quantity = min(abs(old_quantity), quantity)
            if old_quantity > 0:
                position.realized_pnl_ticks += closing_quantity * (price_ticks - old_average)
            else:
                position.realized_pnl_ticks += closing_quantity * (old_average - price_ticks)
            position.quantity = old_quantity + signed_quantity
        ...
        return Position(position.quantity, position.average_price_ticks, position.realized_pnl_ticks, position.last_price_ticks)

    def validate_order(self, instrument_id: int, side: int, quantity: int, price_ticks: int) -> tuple[bool, str]:
        if side not in (-1, 1) or quantity <= 0 or price_ticks <= 0:
        ...
        current_gross = sum(position.gross_quantity for position in self.positions.values())
        current_notional = sum(position.gross_quantity * position.last_price_ticks for position in self.positions.values())
        current = self.positions[instrument_id]
        projected_net = current_net + side * quantity
        projected_gross = current_gross - current.gross_quantity + abs(current.quantity + side * quantity)
        projected_notional = current_notional - current.gross_quantity * current.last_price_ticks + abs(current.quantity + side * quantity) * price_ticks
    ...
    def snapshot(self, prices: Mapping[int, int] | None = None) -> dict[str, object]:
        ...
        gross_position = sum(position.gross_quantity for position in self.positions.values())
        gross_notional = 0
        realized_pnl = 0
        unrealized_pnl = 0
        ...
        for instrument_id, position in self.positions.items():
            ...
            if position.quantity > 0:
                unrealized_pnl += position.quantity * (mark - position.average_price_ticks)
            elif position.quantity < 0:
                unrealized_pnl += abs(position.quantity) * (position.average_price_ticks - mark)
        ...
        return {
            ...
            "positions": {str(instrument_id): position.__dict__.copy() for instrument_id, position in self.positions.items()},
        }
