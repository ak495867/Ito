...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "usage: render_benchmark_suite.py <suite.json> <output_dir>",
            file=sys.stderr,
        )
        return 2
    ...
    for venue in venue_names:
        axis.plot(
            jitter,
            [scenario["venues_detail"][venue]["p99_ns"] for scenario in scenarios],
            marker="o",
            label=venue,
        )
    ...
    lines = [
        ...
        "",
        f"The suite evaluated **{suite['parameters']['branches']}** branches, **{suite['parameters']['messages_per_branch']}** messages per branch and adapter per scenario, and **{len(venue_names)}** venue adapters across jitter levels of {', '.join(str(value) + ' ns' for value in jitter)}.",
        "",
        "## Venue p99 latency by jitter",
        "",
        "| Jitter | " + " | ".join(venue_names) + " | Aggregate |",
        "| ---: | " + " | ".join("---:" for _ in venue_names) + " | ---: |",
    ]
    for scenario in scenarios:
        lines.append(
            "| "
            + str(scenario["jitter_ns"])
            + " ns | "
            + " | ".join(
                str(scenario["venues_detail"][venue]["p99_ns"]) + " ns"
                for venue in venue_names
            )
            + " | "
            + str(scenario["aggregate"]["p99_ns"])
            + " ns |"
        )
    ...
    for venue in venue_names:
        ...
        lines.append(
            f"| {venue} | {start} ns | {end} ns | {increase} ns | {percentage}% |"
        )
    lines.extend(
        [
            "",
