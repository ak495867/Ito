...
def grouped_bars(axis, labels, detail, title):
    ...
    width = 0.24
    axis.bar(
        [position - width for position in positions], p50, width=width, label="p50"
    )
    axis.bar(positions, p95, width=width, label="p95")
    axis.bar(
        [position + width for position in positions], p99, width=width, label="p99"
    )
...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "usage: render_benchmark_report.py <benchmark.json> <output_dir>",
            file=sys.stderr,
        )
        return 2
    ...
    fig, axes = plt.subplots(
        2 if venues else 1, 1, figsize=(12, 9 if venues else 5), dpi=160
    )
    axes = (
        [axes]
    ...
    grouped_bars(
        axes[0], list(branches), branches, "Ito branch latency under simulated jitter"
    )
    if venues:
        grouped_bars(
            axes[1],
            list(venues),
            venues,
            "Ito venue-adapter latency under simulated jitter",
        )
    ...
    lines = [
        ...
        "",
        f"The benchmark processed **{data['messages']}** messages across **{data['branches']}** branches and **{data['venues']}** venue adapters under deterministic simulated network jitter. The exchange simulator status was `{data['exchange_simulator']}`.",
        "",
        "## Aggregate results",
        "",
        "| Metric | Value |",
        ...
        f"| Throughput | {data['throughput_messages_per_second']} messages/second |",
        f"| p50 | {data['aggregate']['p50_ns']} ns |",
        f"| p95 | {data['aggregate']['p95_ns']} ns |",
        f"| p99 | {data['aggregate']['p99_ns']} ns |",
        f"| Maximum | {data['aggregate']['max_ns']} ns |",
        "",
    ...
    for label, value in venues.items():
        lines.append(
            f"| {label} | {value['messages']} | {value['mean_ns']} ns | {value['p50_ns']} ns | {value['p95_ns']} ns | {value['p99_ns']} ns | {value['max_ns']} ns |"
        )
    lines.extend(
        [
            "",
