...
def run_suite(
            ...
            for venue_number, venue_id in enumerate(venues):
                samples.extend(
                    branch_samples(
                        ...
                        messages_per_branch,
                        interval_ns,
                        base_latency_ns + branch_number * 250 + venue_number * 1000,
                        jitter_ns + venue_number * 200,
                        rng,
                        venue_id,
    ...
    return {
        "parameters": {
            "branches": branches,
            "messages_per_branch": messages_per_branch,
            "interval_ns": interval_ns,
            "base_latency_ns": base_latency_ns,
            "jitter_levels_ns": jitter_levels,
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("docs/benchmarks/multibranch_suite.json")
    )
    args = parser.parse_args()
    jitter_levels = [
        int(value) for value in args.jitter_levels_ns.split(",") if value.strip()
    ]
    venues = [
