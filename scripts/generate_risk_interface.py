...
def validate_registers(register_map: dict[str, object]) -> tuple[int, list[dict[str, object]]]:
    ...
    for register in registers:
        ...
        normalized.append({"name": name, "offset": offset, "bytes": width, "access": str(register.get("access", "")), "fields": register.get("fields", [])})
    return frame_bytes, normalized


def main() -> int:
    root = Path(__file__).resolve().parents[1]
    register_map = json.loads((root / "interfaces/register_maps/risk_gate.json").read_text(encoding="utf-8"))
    ...
    rust_lines = [f"pub const REGISTER_MAP_VERSION: u64 = {version};", f"pub const FRAME_BYTES: usize = {frame_bytes};", f"pub const FRAME_BITS: usize = {frame_bits};"]
    sv_lines = ["package ito_risk_frame_pkg;", f"localparam integer REGISTER_MAP_VERSION = {version};", f"localparam integer FRAME_BYTES = {frame_bytes};", f"localparam integer FRAME_BITS = {frame_bits};"]
    for register in registers:
    ...
    generated_json = {"register_map_version": version, "frame_bytes": frame_bytes, "frame_bits": frame_bits, "registers": registers}
    (root / "rust/risk_service/src/generated.rs").write_text("\n".join(rust_lines) + "\n", encoding="utf-8")
    (root / "rtl/common/generated/risk_frame_pkg.sv").write_text("\n".join(sv_lines) + "\n", encoding="utf-8")
    (root / "interfaces/generated/risk_frame_layout.json").write_text(json.dumps(generated_json, indent=2, sort_keys=True) + "\n", encoding="utf-8")
