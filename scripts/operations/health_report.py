...
def build_report(readiness_path: Path, recovery_path: Path) -> dict[str, object]:
    ...
    local_ids = {"GATE-SW-001", "GATE-SW-002", "GATE-SW-003", "GATE-SW-004", "GATE-HW-001", "GATE-OPS-001", "GATE-SEC-003", "GATE-SEC-004", "GATE-OPS-003", "GATE-OPS-004", "GATE-OPS-005"}
    if isinstance(readiness.get("gates"), list):
        gates = readiness["gates"]
        local_failures = [str(gate.get("id")) for gate in gates if isinstance(gate, dict) and gate.get("id") in local_ids and gate.get("implemented") is not True]
        blockers = [str(gate.get("id")) for gate in gates if isinstance(gate, dict) and gate.get("implemented") is not True]
    elif isinstance(readiness.get("blockers"), list):
        blockers = [str(gate.get("id")) for gate in readiness["blockers"] if isinstance(gate, dict)]
        local_failures = [identifier for identifier in blockers if identifier in local_ids]
    else:
        raise HealthError("invalid_readiness")
    return {
        "format": 1,
        "local_status": "healthy" if not local_failures and recovery_path.is_file() else "degraded",
